...
const TextArea = styled.textarea`
	...
	&:focus {
		background: ${({ theme }) => theme.palette.gray4.regular};
		outline: none;
	}
`;
...
const TAContainer = styled(Container)`
	...
	&:focus-within {
		background: ${({ theme }) => theme.palette.gray4.regular};
		outline: none;
		border-bottom: 1px solid ${({ theme }) => theme.palette.primary.regular};
	}
`;
...
const LabelContainer = styled(Container)`
	border-bottom: 1px solid ${(props) => (props.disabled ? 'red' : '#cfd5dc')};
`;
...
function ModuleLabelFactory({ selected, label, open, focus }) {
	return (
		<LabelContainer
...
function _LabelFactory({ selected, label, open, focus, showErr }) {
	return (
		<LabelContainer
			disabled={showErr}
			orientation="horizontal"
...
export default function Feedback({ reporter }) {
	...
	const apps = useMemo(
		() => [
			{ label: 'Mail', value: 'carbonio-mails-ui' },
			{ label: 'Calendar', value: 'carbonio-calendars-ui' },
			{ label: 'Contacts', value: 'carbonio-contacts-ui' },
			{ label: 'Files', value: 'carbonio-files-ui' },
			{ label: 'Chats', value: 'carbonio-chats-ui' },
			{ label: 'Other', value: 'carbonio-shell-ui' }
		],
		[]
	);
	...
	const onAppSelect = useCallback(
		(ev) =>
			dispatch({
				...
				payload: {
					app: ev,
					version: find(acct.apps, ['package', ev])?.version
				}
			}),
		[acct]
	);
	...
	const confirmHandler = useCallback(() => {
		const feedback = reporter.reportFeedback(event);
		...
		closeBoard();
		setAppContext({ disabled: false });
	}, [reporter, event, closeBoard]);
	...
	useEffect(() => {
		updateBoard(undefined, 'Feedback');
		return () => {
			setAppContext({ disabled: false });
		};
	}, [updateBoard]);
	...
	const disabledSend = useMemo(
		() => event.message.length <= 0 || event.extra.topic === '0' || event.extra.app === '0',
